...
const OtherSteps = ({ data, defaultImg }) => {
    ...
    return (
        <div>
            <Progress2 step={data?.step} />
            <div className="d-flex flex-column justify-content-center align-items-center text-center">

                <div className="my-md-5 my-4">
                    {data?.step !== 1 && <h6 className="text-uppercase fw-normal stage-text">
                        Stage {data?.stage} – {data?.scenario}
                    </h6>}
                    <h2 className="text-white fw-bold">{data?.question}</h2>
                </div>

                {data?.step !== 1 && <p className="text-light mb-4 h6">Please Select the Correct
Answer</p>}
                    ...
                    <div className="row justify-content-center">
                        {shuffle1?.map((answer, idx) => (
                            <div key={idx} className="col-12 col-md-4 col-lg-3 mb-3">
                                <div
                                    className={`card step-second-answer-box ${selected === answer?.value
                                        ? "bg-dark text-white"
                                        : "bg-white light-back-txt"
                                        }`}
