...
import "./CounselorCalendar.css";
import { v4 as uuidv4 } from "uuid";

const CounselorCalendar = forwardRef(
  (
    {
      height = "800px",
      defaultView = "month",
    ...
    useEffect(() => {
      calendarInstRef.current = new TuiCalendar(tuiRef.current, {
        ...
        template: {
          ...
          allday: function (schedules) {
            return _getTimeTemplate(schedules, true);
          },
          alldayTitle: function () {
            return '<span class="tui-full-calendar-left-content">ALL DAY</span>';
          },
          time: function (schedules) {
            return _getTimeTemplate(schedules, false);
          },
    ...
    function _getTimeTemplate(schedule, isAllDay) {
      var html = [];
      var viewName = calendarInstRef.current.getViewName(); // Get the current view name
      ...
      if (!isAllDay) {
        ...
        html.push(
          "<strong>" +
            moment(schedule.start.toDate()).format("YYYY-MM-DD") +
            "</strong> "
        );
      }
      ...
      // Conditionally add body based on the view name
      if (viewName === "week" || viewName === "day") {
        if (schedule.body) {
          const truncatedBody =
            schedule.body.length > 6
              ? schedule.body.substring(0, 16) + "..."
              : schedule.body;
          html.push("<br><span>" + truncatedBody + "</span>");
        }
      }
    ...
    function createSchedule(schedule) {
      const newSchedule = {
        ...
        attendees: [currentUser?.name?.[0]],
      };
      console.log("createSchedule", newSchedule);
      calendarInstRef.current.createSchedules([newSchedule]);
      const cloneFilterSchedules = [...filterSchedules];
      setFilterSchedules((prevState) => [...cloneFilterSchedules, newSchedule]);
    }
    function updateSchedule(schedule, changes) {
      ...
      const updatedSchedules = filterSchedules.map((item) =>
        item.id === schedule.id ? { ...item, ...changes } : item
      );

      // Update state with the modified schedules
      setFilterSchedules(updatedSchedules);

      // Update the calendar directly
      calendarInstRef.current.clear();
      calendarInstRef.current.createSchedules(updatedSchedules);
      calendarInstRef.current.render();
    }
    ...
    const formattedRange = renderRange.split(".").reverse().join(".");
    ...
    return (
      <div>
        <div id="right" style={{ left: !showSlidebar && 0 }}>
          ...
          <div
            className="counsel-calendaer-div"
            ref={tuiRef}
            style={{ height }}
          />
        </div>
